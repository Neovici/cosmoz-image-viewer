...
const EMPTY = 'empty',
	...
	pinchingReducer = (state, action) => {
		...
		case 'pointermove': {
			...
			// Rubber-band zoom first, then derive pan from rubber-banded zoom to keep focal point fixed
			const maxZoom = getMaxZoom(state.iw, state.ih, state.cw, state.ch);
			const rubberBandedZoom = rubberBand(result.zoom, 1, maxZoom);
			const zoomRatio = rubberBandedZoom / result.zoom;
			const rubberBandedPanX = midOriginX + (result.panX - midOriginX) * zoomRatio;
			const rubberBandedPanY = midOriginY + (result.panY - midOriginY) * zoomRatio;

			return {
				...result,
				zoom: rubberBandedZoom,
				panX: rubberBandedPanX,
				panY: rubberBandedPanY,
			};
		}
		case 'pointerup': {
			const remaining = removePointer(state.pointers, action.id);
	...
	loadedReducer = (state, action) => {
		...
		case 'wheel': {
			...
			return clampPan({
				...result,
				zoom: getBounded(result.zoom, 1, maxZoom),
			});
		}
		case 'pointerdown':
		case 'pointermove':
