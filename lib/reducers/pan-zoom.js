...
const EMPTY = 'empty',
	...
	/**
	 * Apply rubber-band resistance to pan values past bounds.
	 * Inside bounds: follows exactly. Past bounds: each additional px
	 * only moves by 1/factor — the further past, the more it resists.
	 */
	rubberBand = (value, min, max, factor = 3) => {
		if (value < min) {
	...
	panningReducer = (state, action) => {
		...
		case 'pointerup': {
			...
			if (Object.keys(remaining).length === 0) {
				// Check if zoom needs snap-back (e.g. after interrupting a settle)
	...
	pinchingReducer = (state, action) => {
		...
		case 'pointermove': {
			...
			const rawScaleDiff = oldPinch.distance
				? newPinch.distance / oldPinch.distance
				: 1;
			const dPanX = newPinch.midX - oldPinch.midX;
			const dPanY = newPinch.midY - oldPinch.midY;

			// Clamp zoom first to get the effective scale change,
			// so pan doesn't drift when zoom is capped at the overshoot limit
			const maxZoom = getMaxZoom(state.iw, state.ih, state.cw, state.ch);
			const clampedZoom = getBounded(
				state.zoom * rawScaleDiff,
				1 - OVERSHOOT,
				maxZoom * (1 + OVERSHOOT),
			);
			const effectiveScaleDiff = clampedZoom / state.zoom;
	...
	settlingReducer = (state, action) => {
		...
		case 'pointermove': {
			// Don't pan during settling, but keep pointer positions current
			// so the first delta after transitioning out isn't stale
			if (!state.pointers[action.id]) {
				return state;
			}
			return {
				...state,
		...
		case 'pointerdown': {
			// Freeze at the current spring zoom and start interacting
			const currentZoom = action.currentZoom ?? state.settleTargetZoom;
			const ratio = state.settleStartZoom !== 0
				? currentZoom / state.settleStartZoom
				: 1;
			const panX =
				state.settleOriginX +
				(state.settleStartPanX - state.settleOriginX) * ratio;
