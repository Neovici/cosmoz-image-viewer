...
import { useSelectedAttachment, useSource } from './use-source';
	...
	useCosmozImageViewer = (host) => {
		const { showZoom, title, loop, detachedShowZoom } = host,
			...
			{ selected, value, onSelect } =
				useSelectedAttachment(attachments),
			pdfUrl = selected?.pdf,
			...
			{ slide, next, prev, goto, index, first, last } = useSlideList(
				images,
				{
					loop,
			...
			detach = useCallback(
				...
				[images, index, title, loop, selected],
			),
