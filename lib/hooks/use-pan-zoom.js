 ...
 */
	usePanZoom = (panStiffness = 0.3, zoomStiffness = 0.08) => { // eslint-disable-line max-statements
		const [state, dispatch] = useReducer(panZoomReducer, initState),
			...
			loaded = useCallback(dimensions => dispatch({ type: 'loaded', dimensions }), [dispatch]),
			error = useCallback(() => dispatch({type: 'error'}), [dispatch]),
			zoomBy = useCallback((delta, bounded = true) => dispatch({type: 'zoomBy', delta, bounded}), [dispatch]),
			zoomTo = useCallback(zoom => dispatch({type: 'zoom', zoom}), [dispatch]),

			containerRef = useRef(),
			onResize = useCallback(rect => rect && dispatch({type: 'resize', rect}), [dispatch]),
