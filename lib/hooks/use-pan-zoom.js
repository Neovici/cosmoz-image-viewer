...
const getStyle = (opts, zoom, panX, panY) => {
	...
	/**
	 * Converts viewport coordinates to container-center-relative coordinates.
	 */
	toContainerOrigin = (clientX, clientY, containerRef) => {
	...
	usePanZoom = (panStiffness = 0.3, zoomStiffness = 0.08) => {
		const [state, dispatch] = useReducer(panZoomReducer, initState),
			...
			resize = useCallback(
				() => onResize(containerRef.current && containerRef.current.getBoundingClientRect()),
		...
		// Apply pinch zoom deltas with origin
		useEffect(() => {
		...
		// Snap-back: when pinch ends, snap zoom to bounded range (no origin — pan stays put)
		const wasPinchingRef = useRef(false),
			settlingRef = useRef(false);
		useEffect(() => {
			if (pinching) {
		...
		useMemo(() => {
			if (mouseDX !== 0 || mouseDY !== 0) {
				dispatch({ type: 'pan', dx: mouseDX, dy: mouseDY });
			}
		}, [mouseTS]);

		// Accumulate touch pan deltas (suppress during pinch and settling)
		useMemo(() => {
			if (pinching || settlingRef.current) return;
			if (touchDX !== 0 || touchDY !== 0) {
				dispatch({ type: 'pan', dx: touchDX, dy: touchDY });
			}
		}, [touchTS]);

		// Clamp pan to bounds when panning stops
		useEffect(() => {
			if (!panning) {
		...
		// Match pan spring stiffness to zoom during post-pinch settling
		// so pan and zoom converge together and the focal point doesn't drift
		let panStiff = panStiffness;
		if (settlingRef.current) {
			panStiff = zoomStiffness;
		} else if (panning) {
			panStiff = 0.9;
		}
		const
			springX = useSpring(state.panX, panStiff),
			springY = useSpring(state.panY, panStiff),
			springZoom = useSpring(state.zoom, pinching ? 0.8 : zoomStiffness);

		// Clear settling when zoom spring has converged, then clamp pan for final zoom
		if (settlingRef.current && Math.abs(springZoom - state.zoom) < 0.001) {
