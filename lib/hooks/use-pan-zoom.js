...
const getStyle = (opts, zoom, panX, panY) => {
	...
	usePanZoom = (panStiffness = 0.3, zoomStiffness = 0.08) => {
		...
		// Clamp pan to bounds when interaction stops (skip during settling — snap-back already clamped)
		useEffect(() => {
			if (!panning && !settlingRef.current) dispatch({ type: 'panEnd' });
		}, [panning]);
		...
		// Clear settling when zoom spring has converged
