...
`,Yn=t=>{const{placement:e="bottom span-right",disabled:i,openOnHover:s,openOnFocus:n}=t,r=ie(),[o,a]=Fe("opened",!1),l=P(()=>{i||(a(!0),r.current?.showPopover())},[i]),c=P(()=>{a(!1),r.current?.hidePopover()},[]),u=P(()=>{if(i)return;r.current?.matches(":popover-open")?c():l()},[i]);z(()=>{const g=r.current;g&&(o?g.showPopover():g.hidePopover())},[o]),z(()=>{t.toggleAttribute("opened",!!o)},[o]);const{scheduleClose:d,cancelClose:h}=Bn({host:t,popoverRef:r,disabled:i,openOnHover:s,openOnFocus:n,open:l,close:c}),p=n?l:u,f=P(g=>{Hn(g),a(g.newState==="open"),t.dispatchEvent(new ToggleEvent("dropdown-toggle",{newState:g.newState,oldState:g.oldState,composed:!0}))},[]);return v`
	...
	`};customElements.define("cosmoz-dropdown-next",U(Yn,{styleSheets:[Un],observedAttributes:["placement","disabled","open-on-hover","open-on-focus"],shadowRootInit:{mode:"open",delegatesFocus:!0}}));const J=t=>t??G;const xt=se(class extends we{constructor(t){if(super(t),t.type!==Z.PROPERTY&&t.type!==Z.ATTRIBUTE&&t.type!==Z.BOOLEAN_ATTRIBUTE)throw Error("The `live` directive is not allowed on child or event bindings");if(!ms(t))throw Error("`live` bindings can only contain a single expression")}render(t){return t}update(t,[e]){if(e===H||e===G)return e;const i=t.element,s=t.name;if(t.type===Z.PROPERTY){if(e===i[s])return H}else if(t.type===Z.BOOLEAN_ATTRIBUTE){if(!!e===i.hasAttribute(s))return H}else if(t.type===Z.ATTRIBUTE&&i.getAttribute(s)===e+"")return H;return _s(t),e}});function k(t,e,i){return t?e(t):i?.(t)}const Cs=(t,{label:e,invalid:i,errorMessage:s})=>v`
...
`,Xn=t=>M(()=>{if(t==null)return;const e=new RegExp(t,"u");return i=>{!i.defaultPrevented&&i.data&&!e.test(i.data)&&i.preventDefault()}},[t]),li=te(class extends ee{values;constructor(t,e,i,s){super(t,e),Object.assign(e.host,i),this.values=s}update(t,e){this.hasChanged(e)&&(this.values=e,Object.assign(this.state.host,t))}hasChanged(t=[]){return t.some((e,i)=>this.values[i]!==e)}}),Xe=te(class extends ee{update(){return this.state.host}}),Kn=/([A-Z])/gu,Wt=(t,e,i)=>{t[e]=i,t.dispatchEvent(new CustomEvent(e.replace(Kn,"-$1").toLowerCase()+"-changed",{detail:{value:i}}))},Jn=(t,e,i=[e])=>{const s=Xe();z(()=>{Wt(s,t,e)},i)},$s=t=>{const e=ie(void 0),i=P(l=>e.current=l,[]),s=t.shadowRoot,n=P(l=>t.dispatchEvent(new Event(l.type,{bubbles:l.bubbles})),[]),r=P(l=>Wt(t,"value",l.target.value),[]),o=P(l=>Wt(t,"focused",l.type==="focus"),[]),a=P(()=>{const l=e.current?.checkValidity();return t.toggleAttribute("invalid",!l),l},[]);return li({validate:a},[a]),z(()=>{const l=c=>{c.composedPath()[0]?.closest?.("input, textarea")||(c.preventDefault(),e.current?.focus())};return s.addEventListener("mousedown",l),()=>s.removeEventListener("mousedown",l)},[]),{onChange:n,onFocus:o,onInput:r,onRef:i}},Wn=({placeholder:t,noLabelFloat:e,label:i})=>(e?i:void 0)||t||" ",Gn=["type","pattern","allowed-pattern","min","max","step","autosize","label","placeholder",...Es],Zn=t=>{const{type:e="text",pattern:i,allowedPattern:s,autocomplete:n,value:r,readonly:o,disabled:a,min:l,max:c,step:u,maxlength:d}=t,{onChange:h,onFocus:p,onInput:f,onRef:g}=$s(t),m=Xn(s);return Cs(v`
		...
		`,t)};customElements.define("cosmoz-input",U(Zn,{observedAttributes:Gn,styleSheets:[oi(Os)],shadowRootInit:{mode:"open",delegatesFocus:!0}}));const Oi=t=>{t.style.height="",t.style.height=`${t.scrollHeight}px`},qn=(t,e=0)=>{if(e>0){const i=t.getAttribute("rows")??"",s=t.style.height;t.style.height="",t.setAttribute("rows",e),t.style.maxHeight=t.getBoundingClientRect().height+"px",t.style.height=s,t.setAttribute("rows",i)}},Qn=t=>{const{value:e,maxRows:i}=t,s=M(()=>()=>t.shadowRoot.querySelector("#input"),[]);z(()=>qn(s(),i),[i,s]),z(()=>Oi(s()),[s,e]),z(()=>{const n=s(),r=new ResizeObserver(()=>requestAnimationFrame(()=>Oi(n)));return r.observe(n),()=>r.unobserve(n)},[s])},er=["rows","placeholder",...Es],tr=t=>{const{autocomplete:e,value:i,placeholder:s,readonly:n,disabled:r,rows:o,cols:a,maxlength:l}=t,{onChange:c,onFocus:u,onInput:d,onRef:h}=$s(t);return Qn(t),Cs(v`
			<textarea id="input" part="input"
				...
				.value=${xt(i??"")} maxlength=${J(l)} @input=${d}
		...
		<slot name="suffix"></slot>
		${k(n,o=>v`<div class="failure">${o}</div>`)} `},sr=fe`
	.toggle {
		appearance: none;
		width: 35px;
		height: 18px;
...
`,Qr=({index:t,itemHeight:e,auto:i})=>fe`
	${k(!i,()=>fe`
			.item {
				line-height: ${e}px;
				height: ${e}px;
...
`,eo=t=>{const e=t==="auto",[i,s]=Q(e?40:t);return[i,n=>e?s(n):void 0]},Rs=Vn(()=>As);customElements.define("cosmoz-keybinding-provider",Rs.Provider);const ui=t=>{const e=M(()=>({}),[]);return M(()=>Object.assign(e,t),[e,...Object.values(t)])},Ie=(t,e)=>{const i=xs(Rs),s=ui(t);z(()=>i(s),e)},to=Symbol("listbox.navigate.up"),io=Symbol("listbox.navigate.down"),so=Symbol("listbox.select"),no=({onUp:t,onDown:e,onEnter:i})=>{const s=Xe();Ie({activity:to,callback:t,element:()=>s},[]),Ie({activity:io,callback:e,element:()=>s},[]),Ie({activity:so,callback:i,element:()=>s},[])},ro=({items:t,onSelect:e,defaultIndex:i=0})=>{const[s,n]=Q({index:i}),{index:r}=s,{length:o}=t;return z(()=>{n({index:s.index<0?i:Math.min(s.index,t.length-1),scroll:!0})},[t,i]),no({onUp:P(()=>n(a=>({index:a.index>0?a.index-1:o-1,scroll:!0})),[o]),onDown:P(()=>n(a=>({index:a.index<o-1?a.index+1:0,scroll:!0})),[o]),onEnter:P(()=>r>-1&&r<o&&e?.(t[r],r),[r,t,e])}),{position:s,highlight:P(a=>n({index:a}),[]),select:P(a=>e?.(a),[e])}},oo=t=>typeof t=="object"&&t!==null&&Symbol.iterator in t;function ae(t){return t==null?[]:Array.isArray(t)?t:typeof t=="string"?[t]:oo(t)?Array.from(t):[t]}const Vt=(t,e=ht)=>i=>{const s=ae(t).map(e);return ae(i).filter(n=>!s.includes(e(n)))},ao=(t,e)=>e?i=>i!=null&&ae(t).find(s=>s[e]===i[e]):i=>i!=null&&ae(t).includes(i),lo=(t,e)=>{if(!e||!t)return t;const i=t.toLowerCase().indexOf(e.toLowerCase());if(i<0)return t;const s=i+e.length;return[t.slice(0,i),v`<mark>${t.slice(i,s)}</mark>`,t.slice(s)]},co=(t=ht)=>(e,i,{highlight:s,select:n,textual:r=ht,query:o,isSelected:a})=>{const l=r(e),c=lo(l,o),u=t(c,e,i);return v`<div
				class="item"
				role="option"
				part="option"
				?aria-selected=${a(e)}
...
`;customElements.define("cosmoz-autocomplete-chip",U(yo,{observedAttributes:["disabled"],styleSheets:[ai(mo)]}));const bo=({content:t,onClear:e,disabled:i,hidden:s,className:n="chip",slot:r})=>v`<cosmoz-autocomplete-chip
	...
	>`,vo=({value:t,min:e=0,onDeselect:i,textual:s,disabled:n,chipRenderer:r=bo})=>[...t.filter(Boolean).map(o=>r({item:o,content:s(o),onClear:t.length>e&&(()=>i(o)),disabled:n,slot:"control"})),r({item:null,content:v`<span></span>`,className:"badge",disabled:!0,slot:"control",hidden:!0})],wo=fe`
	:host {
		display: inline-block;
		vertical-align: middle;
...
`;customElements.define("cosmoz-autocomplete-skeleton-span",U(()=>G,{styleSheets:[wo]}));const xo=wt`
	:host {
		display: block;
		position: relative;
		min-width: 35px;
...
`,di=(t,e=()=>({}))=>{const i={type:t,toString(){return t}};return Object.assign((...n)=>Object.assign(e(...n),i),i)},Xi=t=>t.type||t.toString(),Ki=t=>Array.isArray(t)?t:[t],So=(t,e)=>{const i=Ki(e),s=(i.every(Array.isArray)?i:[i]).map(([n,r])=>({actions:Ki(n).map(Xi),handle:r}));return(n=t,r)=>{const o=s.find(a=>a.actions.includes(Xi(r)));return o?o.handle(n,r):n}},he={pending:"pending",rejected:"rejected",resolved:"resolved"},ks={error:void 0,result:void 0,state:he.pending},Ns=di(he.pending),Is=di(he.resolved,t=>({result:t})),Ms=di(he.rejected,t=>({error:t})),Po=So(ks,[[Ns,()=>({error:void 0,result:void 0,state:he.pending})],[Is,(t,{result:e})=>({error:void 0,result:e,state:he.resolved})],[Ms,(t,{error:e})=>({error:e,result:void 0,state:he.rejected})]]),Ds=t=>{const[{error:e,result:i,state:s},n]=Ss(Po,ks);return z(()=>{if(!t)return;let r=!1;return n(Ns()),t.then(o=>!r&&n(Is(o)),o=>!r&&n(Ms(o))),()=>{r=!0}},[t]),[i,e,s]},Co=Symbol("autocomplete.deselect.last"),Eo=Symbol("autocomplete.search.when.selected"),Ji=t=>t.normalize("NFD").replace(/[\u0300-\u036f]/gu,""),Oo=(t,e,i)=>{if(!e)return t;const s=Ji(e.toLowerCase()),n=[];for(const r of t){const a=Ji(i(r).toLowerCase()).indexOf(s);a<0||n.push({item:r,index:a})}return n.sort((r,o)=>r.index-o.index).map(({item:r})=>r)},$o=t=>t===!1||t==null?[]:t,zo=(t,e,i)=>t.dispatchEvent(new CustomEvent(e,{detail:i})),Wi=[],To=t=>(...e)=>{let i;const s=()=>{i&&cancelAnimationFrame(i)};return s(),i=requestAnimationFrame(()=>{i=void 0,t(...e)}),s},Lo=(t,e,i)=>P(s=>{e?.(s),t.dispatchEvent(new CustomEvent(i,{detail:s}))},[e]),Ao=({value:t,text:e,onChange:i,onText:s,onSelect:n,limit:r,min:o,source:a,textProperty:l,textual:c,valueProperty:u,keepOpened:d,keepQuery:h,preserveOrder:p,defaultIndex:f,externalSearch:g,disabled:m,lazyOpen:_})=>{const y=r!=null?Number(r):void 0,C=M(()=>(c??Wr)(l),[c,l]),w=Xe(),[O,b]=Fe("opened",!1),E=!e,L=M(()=>e?.trim(),[e]),N=Lo(w,s,"text"),R=P(S=>{i?.(S,()=>b(!1)),zo(w,"value",S)},[i]),[I,ne]=Q([]),Y=!!(_&&!L),X=M(()=>Y?Promise.resolve([]):Promise.resolve(typeof a=="function"?a({query:L,active:O}):a).then($o),[a,O,L,Y]),j=M(()=>ae(t),[t]);z(()=>X.then(ne),[X]),Ie({activity:Co,callback:()=>{const S=ae(j);S.length>0&&R(S.slice(0,-1))},check:()=>!m&&E&&w.matches(":focus-within"),element:()=>w},[]),Ie({activity:Eo,callback:S=>{const F=ae(j),W=y===1;F.length>0&&W&&S.key.length===1&&R(F.slice(0,-1))},allowDefault:!0,check:()=>!m&&E&&w.matches(":focus-within"),element:()=>w},[y]),z(()=>{!O&&!h&&N("")},[O,h]),z(()=>{w.toggleAttribute("opened",!!O)},[O]);const D=ui({onText:N,onChange:R,value:j,limit:y,min:o,keepQuery:h,keepOpened:d,setOpened:b,onSelect:n,valueProperty:u}),[,,le]=Ds(X);return{limit:y,opened:O,query:L,textual:C,value:j,source$:X,loading:le==="pending",items:M(()=>{if(!O||Y)return Wi;const S=p?I:[...j,...Vt(j,st(u))(I)];return g?S:Oo(S,L,C)},[I,O,L,C,E,j,p,u,g,Y]),onToggle:P(S=>{m||b(S.newState==="open")},[m]),onText:P(S=>{m||(N(S.target.value),b(!0))},[m,N,e,b]),onSelect:P(S=>{if(m)return;D.onSelect?.(S,D);const{onChange:F,onText:W,limit:ce,min:At,value:Rt,keepQuery:kt,keepOpened:Nt,setOpened:It,valueProperty:Se}=D;kt||W(""),Nt||It(!1);const ue=ae(Rt),Pe=st(Se),A=ue.some(Ce=>Pe(Ce)===Pe(S));A&&ue.length===At||F((A?Vt(S,Pe)(ue):[...ue,S]).slice(-ce))},[m,D]),onDeselect:P(S=>{m||D.onChange(Vt(S,st(D.valueProperty))(D.value))},[m,D]),defaultIndex:L!==void 0&&L?.length>0?0:f}},Ro=t=>{const e=t.shadowRoot.querySelectorAll(".chip"),i=t.shadowRoot.querySelector(".badge");i.hidden=!0;for(const a of e)a.hidden=!1;const n=t.shadowRoot.querySelector("cosmoz-input").shadowRoot?.querySelector(".control")?.getBoundingClientRect();let r;for(r=0;r<e.length;r++){const l=e[r].getBoundingClientRect();if(!(l.x+l.width<=n.x+n.width-24))break}const o=e.length-r;for(i.querySelector("span").textContent="+"+o.toString(),i.hidden=o<1;r<e.length;r++)e[r].hidden=!0},ko=({value:t,opened:e,wrap:i,limit:s})=>{const n=Xe(),r=!(i||s===1),o=M(()=>To(()=>Ro(n)),[]),[a,l]=Q(0);ot(()=>{if(!r)return;const c=n.shadowRoot.querySelector("cosmoz-input"),u=new ResizeObserver(d=>{l(d[0].contentRect.width)});return u.observe(c),()=>u.disconnect()},[r]),ot(()=>r?o():void 0,[r,a,e,t])},No=["input","control","label","line","error","wrap"].map(t=>`${t}: input-${t}`).join(),Io=({opened:t,isSingle:e,showSingle:i,hasResultsOrQuery:s})=>!t||e&&!i?!1:s,Mo=t=>{const{opened:e,invalid:i,errorMessage:s,label:n,placeholder:r,disabled:o,noLabelFloat:a,alwaysFloatLabel:l,textual:c,text:u,onText:d,onToggle:h,onDeselect:p,value:f,limit:g,min:m,showSingle:_,items:y,source$:C,loading:w,chipRenderer:O}=t,b=g===1,E=b&&f?.[0]!=null,L=w||y.length>0||u!=null&&u.length>0;return v`<cosmoz-dropdown-next
		...
		>
			...
			>
				...
				${vo({value:f,min:m,onDeselect:p,textual:c,disabled:o,chipRenderer:O})}
			</cosmoz-input>

			${k(Io({opened:e,isSingle:E,showSingle:_,hasResultsOrQuery:L}),()=>go({...t,items:y,multi:!b},k(w,()=>v`<cosmoz-autocomplete-skeleton-span></cosmoz-autocomplete-skeleton-span>`,()=>k(u!=null&&u.length>0&&y.length===0,()=>v`<slot name="no-result">
...
<body>
	<script type="module" src="${e}"><\/script>
...
</html>`),a.close(),yt=t,t.addEventListener("beforeunload",()=>t._onClose?.()),t.addEventListener("beforeunload",()=>yt=null)},rc=async({source:t,fileIndex:e,index:i,syncImageIndex:s,syncFileIndex:n,title:r,loop:o,detachedShowZoom:a,onDetach:l,onClose:c})=>{const u=window.open(void 0,"czimgviewer",sc({height:700,width:800})),d=globalThis["pdfjs-dist/build/pdf"]?.GlobalWorkerOptions?.workerSrc??globalThis.pdfjsLib?.GlobalWorkerOptions?.workerSrc,h={source:t,fileIndex:e,index:i,detachedShowZoom:a,loop:o,title:r,pdfWorkerSrc:d},p={imageIndex:g=>s({detail:{value:g}}),fileIndex:g=>n({detail:{value:g}})};if(_n()&&u.__popoutUpdate){u._onClose?.(),u.__popoutUpdate(h),u.__popoutSync=p,u._onClose=c,l();return}const f=await ic();nc(u,{entryUrl:f,state:h,sync:p,title:r,onDetach:l,onClose:c})},oc=t=>new Promise(e=>{const i=setInterval(()=>{t()&&(e(),clearInterval(i))},100)}),ac=({images:t})=>{const e=window.open(void 0,"czimgviewerprint");ri(v`<style>
				img {
					display: block;
			...
			${t.map(i=>v`<img src="${Ve(Promise.resolve(ci(i)))}" />`)}`,e.document.body),oc(()=>Array.from(e.document.querySelectorAll("img")).every(i=>i.src&&i.complete)).then(()=>e.print()).then(()=>e.close())},Te=[],lc=t=>{const[e,i]=Q({images:Te,loading:!1,error:void 0});return z(()=>{if(!t){i({images:Te,loading:!1,error:void 0});return}const s=t.pdf,n=t.images??Te;if(!s){i({images:n,loading:!1,error:void 0});return}i({images:Te,loading:!0,error:void 0});let r=!1;return bt(async()=>{const{loadPdfThunks:o}=await import("./pdf-loader-B_xPwwqY.js");return{loadPdfThunks:o}},__vite__mapDeps([5,1]),import.meta.url).then(({loadPdfThunks:o})=>o(s,t.pdfOptions)).then(o=>{r||i({images:o,loading:!1,error:void 0})}).catch(o=>{r||i({images:Te,loading:!1,error:o})}),()=>{r=!0}},[t]),e},fs=[],cc=t=>{const e=M(()=>typeof t=="function"?Promise.resolve(t()):void 0,[t]),[i,s,n]=Ds(e);return typeof t!="function"?{files:t??fs,loading:!1,error:void 0}:{files:i??fs,loading:n==="pending",error:s}},uc=50,dc=300,hc=(t,{onSwipeLeft:e,onSwipeRight:i,enabled:s})=>{const n=ie(null);z(()=>{if(!s)return;const r=a=>{if(a.touches.length!==1)return;const l=a.touches[0];n.current={x:l.clientX,y:l.clientY,t:Date.now()}},o=a=>{if(!n.current)return;const l=a.changedTouches[0],c=l.clientX-n.current.x,u=l.clientY-n.current.y,d=Date.now()-n.current.t;n.current=null,!(d>dc)&&(Math.abs(c)<uc||Math.abs(c)<Math.abs(u)||(c<0?e?.():i?.()))};return t.addEventListener("touchstart",r,{passive:!0}),t.addEventListener("touchend",o,{passive:!0}),()=>{t.removeEventListener("touchstart",r),t.removeEventListener("touchend",o)}},[t,e,i,s])},ni=[],pc=t=>t.source!=null?t.source:t.images!=null||t.pdf!=null?[{images:t.images??ni,pdf:t.pdf,pdfOptions:t.pdfOptions}]:ni,yn=t=>{if(!t.currentTarget.parentElement)return;t.currentTarget.parentElement.querySelector(".error").removeAttribute("hidden"),t.currentTarget.setAttribute("hidden",!0)},fc=t=>t.detail.value==="error"&&yn(t),gc=({src$:t,showZoom:e,isZoomed:i})=>{const s=ve(t,()=>Ve(t));return[e?v`<haunted-pan-zoom
						.src=${s}
						?disabled=${!i}
						@status-changed=${fc}
					></haunted-pan-zoom>`:v`<img .src=${s} style="width:100%" @error=${yn} />`,ve(t,()=>Ve(t.then(()=>G),v`<cz-spinner></cz-spinner>`))]},mc=t=>v`<div>
			<div hidden class="error">
				<h2>An error occurred while loading the image.</h2>
				<div class="desc">${t.image}</div>
			</div>
			${gc(t)}
		</div>`,Kt=(t,e,i)=>Math.max(e,Math.min(i,t)),gs=(t,e,i,s)=>{const n=t.length;for(let r=1;r<=n;r++){const o=(e+r*i+n)%n;if(!s&&i>0&&o<=e||!s&&i<0&&o>=e)return;const a=t[o];if((a.images?.length??0)>0||a.pdf)return o}},_c=t=>{const{showZoom:e,title:i,loop:s,detachedShowZoom:n}=t,r=M(()=>pc(t),[t.source,t.images,t.pdf,t.pdfOptions]),{files:o,loading:a,error:l}=cc(r),[c,u]=Fe("currentFileIndex",0),d=o.length>0?Kt(c??0,0,o.length-1):0,h=o[d],p=h?[h]:ni,f=ie(qe),g=P(A=>{const Ce=A.detail?.[0],Je=o.indexOf(Ce);Je>=0&&(f.current=qe,u(Je))},[o]),m=h?.downloadFileName??"archive",{images:_,loading:y,error:C}=lc(h),[w,O]=Q(!1),b=P(()=>(O(A=>!A),w?1:1.5),[w]),E=M(()=>Gl(A=>Promise.resolve(ci(A))),[]),[L,N]=Fe("currentImageIndex",0),R=_.length>0?Kt(L??0,0,_.length-1):0,I=_[R],ne=M(()=>I==null?{id:Math.random(),content:G,animation:qe}:{id:I,render:()=>mc({src$:E(I),showZoom:e,isZoomed:w,image:I}),animation:f.current},[I,e,w]),Y=ie(!1),X=!s&&d===0&&R<=0,j=!s&&d===o.length-1&&R>=_.length-1,D=P(()=>{if(f.current=qe,R<_.length-1)return N(R+1);const A=gs(o,d,1,s);A!=null&&(u(A),N(0))},[R,_.length,o,d,s]),le=P(()=>{if(f.current=Yo,R>0)return N(R-1);const A=gs(o,d,-1,s);A!=null&&(Y.current=!0,u(A))},[R,o,d,s]);hc(t,{onSwipeLeft:D,onSwipeRight:le,enabled:!w});const[S,F]=Q(!1),[W,ce]=Q(!1),At=()=>ce(!0),Rt=()=>ce(!1),kt=A=>F(A.detail.value),Nt=A=>N(A.detail.value),It=A=>u(A.detail.value),Se=P(()=>rc({source:o,fileIndex:d,index:R,syncImageIndex:Nt,syncFileIndex:It,title:i,loop:s,detachedShowZoom:n,onDetach:()=>F(!0),onClose:()=>F(!1)}),[o,d,R,i,s,n]),ue=P(()=>{_n()&&Se()},[Se]);z(()=>{if(_.length===0)return;if(Y.current){Y.current=!1,N(_.length-1);return}const A=L??0;A>=_.length&&N(Kt(A,0,_.length-1))},[_]),z(()=>{const A=c??0;o.length>0&&A>=o.length&&u(0)},[o]),Jn("detached",S,[S]),z(()=>{t.toggleAttribute("hidden",S)},[S]),li({syncState:ue},[ue]);const Pe=_.length+(o.length>1?o.reduce((A,Ce,Je)=>A+(Je===d?0:Ce.images?.length??0),0):0);return{host:t,isZoomed:w,toggleZoom:b,currentSlide:ne,nextImage:D,previousImage:le,first:X,last:j,total:_.length,navTotal:Pe,currentImageIndex:R,selectedImageNumber:R+1,onDownloadPdf:h?.pdf?()=>ec(h.pdf,m,h.pdfOptions):()=>Ql(_,m),isFullscreen:W,openFullscreen:At,closeFullscreen:Rt,onPrintPdf:()=>ac({images:_}),detached:S,detach:Se,syncDetachedState:kt,setFileIndex:u,setImageIndex:N,loading:a||y,error:l,contentError:C,files:o,selectedFile:h,selectedIndex:d,value:p,onSelect:g,images:_}},yc=v`<svg
	viewBox="0 0 24 24"
...
>
	<path
		...
		fill="currentColor"
	></path>
</svg>`,Oc=t=>t?Sc:Pc,re=(t,e,i=1)=>t?e>=i:!1,$c=({host:t,isZoomed:e,toggleZoom:i,currentSlide:s,nextImage:n,previousImage:r,total:o,navTotal:a,first:l,last:c,currentImageIndex:u,selectedImageNumber:d,onDownloadPdf:h,isFullscreen:p,openFullscreen:f,closeFullscreen:g,onPrintPdf:m,detach:_,detached:y,syncDetachedState:C,setFileIndex:w,setImageIndex:O,loading:b,error:E,contentError:L,files:N,selectedIndex:R,value:I,onSelect:ne})=>v`
	<style>
		${Jl}
	</style>

	${y?G:v`
				${k(b,()=>v`<div class="loading">
				...
				${k(!b&&!E,()=>v`
						${k(re(t.showPageNumber,o),()=>v` <div class="counter">
									${d}/${o}
								</div>`)}

						<div class="actions">
							${k(re(t.showNav,a,2),()=>v`
									<button
										class="nav"
										name="prev"
										?disabled=${l}
								...
								`)}
							${k(N.length>1,()=>v`<cosmoz-autocomplete
										text-property="title"
										limit="1"
										.min=${1}
										show-single
							...
							${k(re(t.showZoom,o),()=>v`<button
										class="nav"
										@click=${Y=>Y.target.getRootNode().querySelector("haunted-pan-zoom").zoomTo(i())}
										title="${K("Zoom image")}"
									>
										${Oc(e)}
									</button>`)}
							${k(re(t.showDetach,o),()=>v`<button
										class="nav"
										@click=${_}
							...
							${k(re(t.showFullscreen,o),()=>v`<button
										class="nav"
										@click="${f}"
						...
						${k(L,()=>v`<p class="error">${K("Failed to load PDF.")}</p>`,()=>k(o===0,()=>v`<p>${K("No image loaded.")}</p>`))}
						<cosmoz-slider id="slider" .slide="${s}"></cosmoz-slider>
					`)}
			`}
	${k(p,()=>Kl(v`<cosmoz-image-viewer
				fullscreen
				.source=${N}
				.currentFileIndex=${R}
				.currentImageIndex=${u}
				@current-file-index-changed=${Ci(w)}
				@current-image-index-changed=${Ci(O)}
				@detached-changed=${C}
				show-nav
				show-zoom
				show-close
				...
				?loop=${t.loop}
			></cosmoz-image-viewer>`))}
`,zc=t=>$c(_c(t));customElements.define("cosmoz-image-viewer",U(zc,{observedAttributes:["source","download-file-name","pdf","show-close","show-detach","show-fullscreen","show-nav","show-page-number","show-zoom","detached-show-zoom","loop","title"]}));
